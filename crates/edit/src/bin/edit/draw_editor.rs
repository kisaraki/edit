...
pub fn draw_editor(ctx: &mut Context, state: &mut State) {
    let size = ctx.size();
    // EN: Search and Replace are centered modals, so only the menu and status rows are reserved.
    // 中文：搜尋與取代改為中央對話框，因此只需保留選單列與狀態列。
...
pub fn draw_dialog_search(ctx: &mut Context, state: &mut State) {
    ...
    // EN: Render Search and Replace above the complete editor instead of consuming document rows.
    // 中文：搜尋與取代顯示於完整編輯器上方，不再占用文件列。
    ...
    ctx.modal_begin("search", title);
    {
        ...
        {
            // EN: F3 continues the search while the modal has focus.
            // 中文：對話框取得焦點時，F3 仍可繼續尋找。
            ...
            ctx.table_set_columns(&[48]);
            {
                ...
                // EN: Each translated option gets its own row so it cannot widen the modal.
                // 中文：每個翻譯後的選項各占一列，避免撐寬對話框。
                ctx.table_next_row();
                change |= ctx.checkbox(
                    "match-case",
            ...
            // EN: Explicit buttons keep all operations discoverable in the modal.
            // 中文：以明確按鈕呈現對話框中的所有操作。
