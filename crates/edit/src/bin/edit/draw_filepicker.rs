...
pub fn draw_file_picker(ctx: &mut Context, state: &mut State) {
    ...
    // EN: Only Open receives a lower MRU pane; Save As keeps the original single-pane layout.
    // 中文：僅「開啟舊檔」加入下方最近檔案欄；另存新檔維持原本單欄配置。
    ...
    // EN: Reserve a bordered lower pane even when fewer than five MRU entries exist.
    // 中文：即使最近檔案不足五筆，仍保留有框線的下方固定面板。
    ...
    ctx.attr_intrinsic_size(Size { width, height });
    {
        ...
        if opening_file {
            // EN: Each absolute path is a one-click button in a distinct lower pane.
            // 中文：每一筆絕對路徑都是下方獨立面板中可單擊開啟的按鈕。
